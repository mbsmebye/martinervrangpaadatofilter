...
# Output: closed.json (array of { completionDate, registrationDate })
...
while ($cursor -lt $ToExclusive) {
    ...
    $url = "$BaseUrl/servicepurchaser/$ServicePurchaserId/collectionorders/closed?closedDateFrom=$start&closedDateTo=$end"
    Write-Host "Fetching $start → $end ..." -ForegroundColor Cyan
...
# Write to closed.json as a single JSON array
$outPath = Join-Path -Path (Get-Location) -ChildPath 'closed.json'
